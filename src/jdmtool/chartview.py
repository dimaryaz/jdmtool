...
class ChartHeader:
    ...
    def to_bytes(self):
        db_begin_date = self.db_begin_date.encode().ljust(11, b'\x00')
        return struct.pack('<4i11s', self.checksum, self.MAGIC, self.num_files, self.index_offset, db_begin_date)
...
class ChartRecord:
    ...
    def from_bytes(cls, data: bytes):
        ...
        return cls(name, offset, size, metadata)

    def to_bytes(self):
        return struct.pack('<26s2i6s', self.name, self.offset, self.size, self.metadata)
